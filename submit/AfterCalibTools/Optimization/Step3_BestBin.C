...
void Step3_BestBin(TString Input, int nLine, TString Input2, int nLine2){
  ...
  for(unsigned int i(0); i<values_effH.size(); i++){
    if(values_sbH[i] > tmp1 && values_chiH[i]<1./*&& values_chiH[i]<0.044 && values_effH[i]>0.28*/ ){
	tmp1 = values_sbH[i];
	bin3 = i; cout<<"-->HIGH_ETA LOW S/B bin:  "<<values_sbH[i]<<" bin "<<values_binH[bin3]<<endl;
    }
    if(values_muH[i] < tmp2 && values_chiH[i]<1. /*&& values_chiH[i]<0.044 && values_effH[i]>0.28*/ ){
	tmp2 = values_muH[i];
