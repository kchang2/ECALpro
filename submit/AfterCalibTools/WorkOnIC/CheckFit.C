...
//Usage: .x Total_fit.C+("root://eoscms//eos/cms/store/group/dpg_ecal/alca_ecalcalib/lpernie/ALL_2015A_RAW_RECHIT_SMIC_estimTime_01/iter_0/2015A_calibMap.root",
...
void CheckFit( TString File, TString folder , int EBorEE=0, bool Are_pi0_=true ){
  ...
  for( int EBEE=0; EBEE<2; EBEE++ ){
	...
	//Get Variables
	tree->GetEntry(i);
	float fit_b0_, fit_b1_, fit_b2_, fit_b3_, fit_mean_, fit_sigma_, fit_Snorm_, fit_Bnorm_, Chisqu_, Ndof_, ietaX_, iphiY_, iZ_;
	if(EBEE==0){
	  ...
	  ietaX_=EBieta_; iphiY_=EBiphi_; iZ_=0;
	}
	if(EBEE==1){
	  ...
	  ietaX_=EEix_; iphiY_=EEiy_; iZ_=EEzside_;
	}
	//Background
	...
	if( Chisqu_/Ndof_ > 0.05) nameH += "_HIGH_CHI2_";
